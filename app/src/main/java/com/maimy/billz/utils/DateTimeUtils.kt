...
class DateTimeUtils {
    companion object {
        ...
        fun getFirstDayOfYear(): String {
            val now = LocalDateTime.now()
            val firstDayOfYear = now.withMonth(Month.JANUARY.value).withDayOfMonth(1)
            return formatDate(firstDayOfYear)
        }
        fun getLastDayOfYear(): String {
            val now = LocalDateTime.now()
            val lastDayOfYear = now.withMonth(Month.DECEMBER.value).withDayOfMonth(31)
